...
/**
 * The mutable runtime state of the one behaviour a mob is currently playing. Behaviours themselves are
 ...
 * behaviours draw from (each uses only what it needs); randomness is seeded so every client watching the
...
public final class BehaviorInstance {
    ...
    public float prevX, x;               // iris target x (uniform behaviours)
    ...
    public float prevTint, tint;         // colour blend amount

    // --- params resolved once in onStart (from the seed) ---
    public float[] tintColor;            // colour-change target
