...
/**
 * Server → client sync of a single entity's full eye state: the {@code hasGooglyEyes} flag plus the
 * optional per-mob appearance overrides (see {@link EyeState}). Sent on start-tracking (so a newly
 * watching player gets current state) and whenever the state is mutated mid-life (so changes from
 * shears / dye / redstone appear immediately on every tracking client).
 */
public class EyeStatePacket {
    ...
    public EyeStatePacket(int entityId, boolean hasGooglyEyes, @Nullable CompoundTag overrides) {
    ...
    public static EyeStatePacket decode(FriendlyByteBuf buffer) {
        ...
        return new EyeStatePacket(entityId, hasGooglyEyes, overrides);
    }
