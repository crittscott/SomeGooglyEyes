...
public class HeadInfo {
     ...
     * mobs share one instance. {@code variantRoll} is the entity's stored {@code EyeState} roll — read
     * by the caller so this class stays free of the platform-specific entity-state mechanism.
     */
    public static HeadInfo getHelper(ResourceLocation entityName, LivingEntity entity, float variantRoll) {
        boolean baby = entity.isBaby();
        RuntimeConfig config = ClientEyeConfigs.get(entityName, baby);
        int variant = chooseVariantIndex(config, variantRoll);
     ...
     * {@code variantRoll} is the entity's stored {@code EyeState} roll — read by the caller so this class
     * stays free of the platform-specific entity-state mechanism.
     */
    public static HeadInfo serverHelper(ResourceLocation entityName, LivingEntity entity, float variantRoll) {
