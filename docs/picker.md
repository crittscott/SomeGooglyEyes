...
To keep junk entities out of the grid, list namespaces in `spawnExcludedMods` or specific ids in `spawnExcludedEntities` (same `Picker` section). Both `/sg spawn` and `/sg spawnall` skip anything they match; a namespace-scoped `spawnall` run reports each skipped id so you can see why it's missing.
